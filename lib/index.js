...
export default class PhoneInput extends Component {
  ...
  componentDidMount() {
    ...
    if (textProps.placeholder) {
      this.setState({ formattedNumber: `${this.state.formattedNumber} ${textProps.placeholder}` });
    }
  }
      ...
      <View style={[styles.container, this.props.style]}>
        ...
        <View style={{ flex: 1, marginLeft: this.props.offset || 10 }}>
          <TextComponent
            ...
            onBlur={() =>{
              const { textProps } = this.props;
              if (textProps && textProps.placeholder) {
                const countryData = PhoneNumber.getCountryDataByCode(iso2);
                const prefix = `+${countryData.dialCode}`;
                if (formattedNumber.trim().replace(prefix, '').length === 0) {
                  this.setState({ formattedNumber: `${prefix} ${textProps.placeholder}` });
                }
              }
            }}
